...
> **已废止（历史追溯）：** 本文仅记录 2026-08-19 的环境设计，不代表当前项目方案。当前 Java、Maven、数据库和运行入口以 `AGENTS.md`、`CLAUDE.md` 及 `docs/specs/00-project/` 正式规格为准。
...
| 服务 | 版本 | 地址 | 运行方式 |
| --- | --- | --- | --- |
| PostgreSQL | 12.1 | `127.0.0.1:5433` | 使用现有本地实例，由用户手动管理 |
...
环境盘点时，6379 和 1883 已存在监听进程。下载 Redis 与 Mosquitto 不受影响，但手动启动新实例前，用户需要先处理原占用进程。既有 Redis 仅在用户先验证该实例只绑定 `127.0.0.1`/`::1`，且其认证策略与项目配置一致时才可复用；当前观察到监听 `0.0.0.0/[::]:6379` 的 Redis 不符合该条件，不能直接复用。
