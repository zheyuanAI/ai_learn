...
- `docs/`：计划、规则和项目说明文档入口。
- `docs/openspec/`：显式 OpenSpec 任务使用的协作规则、schema、模板与长期协作 spec。
- `prototype/`：低保真原型与演示页面基线。
- `docs/specs/`：项目级和各业务域的目标规格事实源；不能据此声称代码已经实现。
...
- `docs/openspec/README.md`：OpenSpec 详细协作规则，仅在显式 OpenSpec 任务下读取。本节路径按项目根解析；下方 `docs`、`openspec`、`specs` 章节中的路径按本文件所在 `docs/` 目录解析。
- `README.md`：仓库总说明，给出基础入口和阅读顺序。
- `CONTEXT.md`：已确认的统一业务术语，避免跨领域同义异名。
- `.gitignore`：Git 忽略规则。

## docs

- `docs/README.md`：文档总入口。
- `项目目录结构.md`：当前这份目录与关键文件说明。
- `specs/00-project/正式项目计划.md`：当前一个月开发计划；`plans/十周学习计划-历史.md` 为历史十周学习资料，不作为当前实施入口。
- `superpowers/plans/2026-08-20-project-content-alignment.md`：正式计划生效后的内容同步执行记录。
- `superpowers/plans/2026-06-30-ai-learn-project.md`：已废止的历史计划，仅供变更追溯。
- `rules/AGENTS_General.md`：通用规则模板参考，不直接作为本项目生效规则。
...
- `specs/00-project/正式项目计划.md`：当前一个月开发计划、黄金业务闭环和一期范围。
- `specs/00-project/项目概述.md`：项目目标、范围、非目标和成功标准。
- `specs/00-project/架构设计.md`：服务边界、横切约束和整体架构。
- `specs/00-project/原型与交互说明.md`：原型范围和页面清单。
...
- `概述.md`：该领域的目标和范围。
- `领域模型.md`：核心实体、字段约束、状态机和核心业务规则。
- `接口契约.md`：接口契约、查询过滤、错误码和返回约束。
- `验收标准.md`：验收场景和自动化测试点。
...
- `specs/20-mes/`：MES。
- `specs/30-iot-digital-twin/`：一期 IoT 设备事实；数字孪生仅为二期候选。
- `specs/40-gis-dashboard/`：GIS / 综合看板。
- `specs/50-ai-assistant/`：一期 AI 只读工具和审计；知识库/RAG 仅为二期候选。
...
- `prototype/pages/sales-outbound.html`：销售单、库存预留、拣货、发货页。
- `prototype/pages/work-order.html`：MES 工单、领退料、派工、工序执行、报工和质检页。
- `prototype/pages/device-alarm.html`：设备、状态、告警页。
- `prototype/pages/site-map.html`：厂区地图页。
- `prototype/pages/digital-twin.html`：二期三维展示设计资产，不进入一期导航与验收。
- `prototype/pages/ai-assistant.html`：AI 聊天页。
- `prototype/pages/knowledge-base.html`：二期知识库设计资产，不进入一期导航与验收。
...
- `deploy/docker-compose.yml`：一期本地基础依赖编排文件，当前仅包含 PostgreSQL、Redis、Mosquitto。
- `deploy/local/redis.conf`：本地 Redis 的回环监听与开发配置。
- `deploy/local/mosquitto.conf`：本地 Mosquitto 的回环监听与开发配置。
- `deploy/docker/mosquitto.conf`：Compose 容器使用的 Mosquitto 配置；宿主机端口仍仅映射到回环地址。

## 本地开发环境基线

- 自研项目使用 Java 21、Maven 3.9.1、Node 20 与 PostgreSQL 16（端口 5433）。
- Gateway、Auth、Core、IoT 端口依次为 10001、10002、10003、10004。
- Redis 与 Mosquitto 的项目内手动运行包、校验信息和启动命令见 `runtime/README.md`。
- 参考工程可继续保留各自的 Node 22 与 PostgreSQL 16（端口 5323）配置。

## 维护建议

- 已实现事实以实际代码和配置为准；批准后的目标业务规则维护在 `specs/`，不要散落到多个说明文档。
