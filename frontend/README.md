...
- **阶段 8 AI 流式交互**：`/ai` 与 `/ai/chat` 已接入真实流式页面，使用 `fetch + ReadableStream` 解析命名 SSE，支持工具运行/完成卡片、停止生成、来源、数据时间、模型和请求编号。采购、库存、制造、质量、IoT、综合日报等非销售链问答已完成端到端验证；确认式页面导航已经下线，涉及多个角色的处理建议改为纯文字；审计页与会话历史仍待后续里程碑。
