...
> = {
  "admin.zhang": {
    ...
    permissions: [
      "dashboard:view (综合看板查看)",
      "gis:map:view (厂区二维地图查看)",
      "auth:tenant:manage (租户管理)",
      "ai:chat:query (AI 只读问答)",
  ...
  "buyer.chen": {
    ...
    permissions: [
      "pur:order:view (采购订单查询)",
      "pur:order:create (创建采购订单)",
      "pur:order:submit (提交采购订单)",
      "pur:order:approve (审核采购订单)",
      "pur:quality:return (采购退回供应方决定)",
    ],
    menus: [
      { id: "1", menuCode: "Overview", routePath: "/", menuName: "一期总览", detail: "一条黄金业务闭环", sortOrder: 1 },
  ...
  "wh.operator": {
    ...
    permissions: [
      "pur:receipt:confirm (仓库到货验收与实收)",
      "pur:disposition:confirm (质量处置执行)",
      "pur:putaway:confirm (确认上架存储)",
      "sales:pick:confirm (仓库直接拣货确认)",
      "sales:shipment:confirm (仓库发货出库确认)",
      "mes:material:confirm (生产领料发货确认)",
      "inv:transfer:confirm (库位调拨确认)",
    ],
    menus: [
      { id: "1", menuCode: "Overview", routePath: "/", menuName: "一期总览", detail: "一条黄金业务闭环", sortOrder: 1 },
  ...
  "mes.inspector": {
    ...
    permissions: [
      "pur:quality:inspect (采购到货质检)",
      "pur:quality:release (采购质量放行决定)",
      "mes:workorder:approve (生产工单审核与下达)",
      "mes:execution:manage (工序执行管理)",
      "mes:report:manage (工序报工管理)",
      "mes:quality:inspect (生产工序质检)",
    ],
    menus: [
      { id: "1", menuCode: "Overview", routePath: "/", menuName: "一期总览", detail: "一条黄金业务闭环", sortOrder: 1 },
  ...
  "iot.engineer": {
    ...
    permissions: [
      "iot:device:view (IoT 设备查询)",
      "iot:device:manage (IoT 设备与凭证管理)",
      "iot:telemetry:view (设备遥测与状态查询)",
      "iot:alarm:ack (设备告警确认)",
    ],
    menus: [
      { id: "1", menuCode: "Overview", routePath: "/", menuName: "一期总览", detail: "一条黄金业务闭环", sortOrder: 1 },
