...
- 项目事实以 `specs/00-project/*.md` 和各领域 `specs/*/{overview,domain,api,acceptance}.md` 为准。
- 领域行为变更若影响状态机、错误码、接口字段、权限点或验收条件，必须同步回写现有 `specs/` 文档，不能只停留在 OpenSpec 工件中。
...
- 禁止修改范围
- 必读工件
- 必须同步更新的现有 `specs/`、`prototype/` 或 `docs/` 文件
- 验证要求
- 失败时的停止条件
- 完成后必须回写的文件
